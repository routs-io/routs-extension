...
import type { IStoredWallet, IWallet, IWalletsStore, TypeTagColor, WalletType } from '@/types/wallets'
...
export const useWalletsStore = defineStore('wallets', {
  ...
  actions: {
    ...
    async saveWallets(wallets: IStoredWallet[]) {
      ...
      if (!wallets.length) {
        if (this.requestId) {
          await this.sendMessage('fuel_generateAccounts', [this.wallets.filter((w) => w.type === 'fuel').map(w => w.address)])
          this.requestId = 0
        }
      ...
      const uniqueAddresses = Array.from(new Set([...walletsInStorage.map(w => w.address.toLowerCase()), ...wallets.map(w => w.address.toLowerCase())]))
      const uniqueWallets = uniqueAddresses.map(address => ({
        privateKey: walletsInStorage.find(w => w.address.toLowerCase() === address)?.privateKey ?? wallets.find(w => w.address.toLowerCase() === address)?.privateKey,
        address: walletsInStorage.find(w => w.address.toLowerCase() === address)?.address ?? wallets.find(w => w.address.toLowerCase() === address)?.address,
        tags: Array.from(walletsInStorage.find(w => w.address.toLowerCase() === address)?.tags ?? wallets.find(w => w.address.toLowerCase() === address)?.tags ?? []),
      ...
      if (this.requestId) {
        await this.sendMessage('fuel_generateAccounts', wallets.map(w => w.address))
        this.requestId = 0
      }
      await this.refreshWallets(0)
    },
    ...
    async connectAll() {
      ...
      this.wallets.filter(({ type }) => type === 'evm').forEach(wallet => wallet.status = 'online')
